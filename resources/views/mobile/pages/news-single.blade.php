...
<section class="m-section">
    ...
    <article>
        ...
        <div class="m-img-wrap" style="aspect-ratio:16/10; margin-bottom:1rem;">
            <img src="{{ upload_asset($news->image_path, 'image/placeholder.svg') }}" alt="{{ $news->title }}" loading="lazy">
        </div>
        @endif
        ...
        <div class="m-card" style="display:flex; align-items:center; gap:12px;">
            @if($news->user && $news->user->avatar)
                <img src="{{ upload_asset($news->user->avatar ?? null, 'image/3/1.png') }}" alt="" style="width:48px; height:48px; border-radius:50%; object-fit:cover;">
            @else
