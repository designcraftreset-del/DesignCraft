...
.m-auth__checkbox { display: flex; align-items: center; gap: 10px; margin: 1rem 0; }
.m-auth__checkbox input { width: 20px; height: 20px; accent-color: var(--m-accent); }
.m-auth__checkbox label { margin: 0; font-size: 0.9375rem; color: var(--m-text); }
...
@section('content')
<div class="m-auth">
    <h1 class="m-auth__title">Вход</h1>
    <p class="m-auth__subtitle">Введите данные для входа в аккаунт</p>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <button type="submit" class="m-auth__submit">Войти</button>

        <div class="m-auth__links">
            @if (Route::has('password.request'))
                <a href="{{ route('mobile.password.request') }}">Забыли пароль?</a>
            @endif
            <span style="margin:0 6px;">·</span>
            <a href="{{ route('mobile.register') }}">Создать аккаунт</a>
        </div>
    </form>
</div>
