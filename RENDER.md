...
6. После успешного импорта:
   ...
   - Добавьте `SETUP_ADMIN_TOKEN`, откройте `/setup-admin?token=...` один раз (пароль из дампа может не подходить), затем удалите `SETUP_ADMIN_TOKEN`.
   - Маршрут `/import-db` оставлен в коде для повторного импорта при необходимости.
...
3. **Если после импорта БД не входится под админом** — пароль перезаписался из дампа. Снова добавьте `SETUP_ADMIN_TOKEN`, откройте `/setup-admin?token=...` один раз, затем удалите переменную.
...
- **Не входится под админом после импорта** — снова выполните шаг 2 (SETUP_ADMIN_TOKEN и `/setup-admin?token=...`).
- **500 после регистрации или в админ-панели** — задеплойте последний код (middleware «последний визит» исправлен), затем проверьте снова.
- **500 в дашборде админки** (графики заказов) — в коде учтена PostgreSQL (EXTRACT вместо MONTH/YEAR); задеплойте последнюю версию.
- **500 в чате поддержки** — после деплоя обновите страницу (в коде учтены пустые даты сообщений).
- **Админка → Сообщения: мигание** — исправлено: опрос подгружает только новые сообщения, без перерисовки всего списка. При ошибке отправки ответа показывается уведомление.
- **Просмотр БД как в phpMyAdmin** — в Render нет встроенного интерфейса; используйте pgAdmin, DBeaver или TablePlus с External Database URL.
- **Другая ошибка 500** — смотрите **Logs** в Render в момент повторения действия.
