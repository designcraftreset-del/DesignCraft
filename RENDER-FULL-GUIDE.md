...
2. Создайте/обновите админа: добавьте `SETUP_ADMIN_TOKEN`, откройте `/setup-admin?token=...` один раз (пароль из дампа может не подходить), затем удалите `SETUP_ADMIN_TOKEN`.
3. Маршрут `/import-db` оставлен в коде для повторного импорта при необходимости; при желании можно не удалять.
...
**Если после импорта БД (вариант B) не входится под админом** — пароль в базе перезаписался из дампа. Снова добавьте `SETUP_ADMIN_TOKEN` в Environment, откройте `/setup-admin?token=...` один раз: пароль для `ii5543135@gmail.com` обновится. Затем удалите переменную.
...
- **Не входится под админом после импорта БД** — снова добавьте `SETUP_ADMIN_TOKEN`, откройте `/setup-admin?token=...`, затем удалите переменную.
- **500 после регистрации или при открытии админ-панели** — в коде исправлен middleware обновления «последний визит». Убедитесь, что задеплоена последняя версия (`git push origin main`), затем попробуйте снова.
- **500 в дашборде админки** (графики по заказам) — исправлено для PostgreSQL (EXTRACT). Задеплойте последний код.
- **500 в чате поддержки** — исправлено в коде (безопасная работа с датами сообщений). После деплоя обновите страницу.
- **Админка → Сообщения: мигание списка** — исправлено: при опросе подгружаются только новые сообщения, без перерисовки всего блока. При ошибке отправки ответа в поддержку показывается alert с текстом ошибки.
- **Просмотр таблиц БД** — в Render нет встроенного phpMyAdmin; используйте pgAdmin, DBeaver или TablePlus с External Database URL (и добавьте свой IP в Allow connections from).
- **Любая другая 500** — откройте в Render: Web Service → **Logs**; в момент повторения действия посмотрите текст ошибки и стек — по нему можно найти причину.
